...
class RankAdapter:
    ...
    Key Features:
        - **Ranking Unit Adaptation**: Adjusts the ranking focus to a domain-specific unit
          (e.g., "doctor" for medical ranking tasks).
    ...
    def _generate_adaptive_unit_and_criteria(self, query: str, instruction: str):
        ...
        Args:
            query (str): The user query describing the ranking task.
            instruction (str): The task instruction specifying the ranking goal.
    ...
    def get_adaptive_ranking_prompt(self, query: str, instruction: str, passages: str, prompt_template: str):
        ...
        The method dynamically adjusts the prompt content based on the `mode`:
            ...
            - **cot**: Adds a "chain of thought" (CoT) process to define criteria first, then apply them
              iteratively to the ranking units.

        Args:
            query (str): The user query defining the retrieval task (e.g., "Find doctors skilled
                         in a specific treatment.").
            instruction (str): Task instructions specifying what needs to be ranked or retrieved.
        ...
        if self.mode == "instruction":
            ...
            prompt_template = prompt_template\
                .replace(". Do not explain any reason.", "and your decision criteria. ")\
                .replace("，无需解释原因。", "和所依据的决策标准。")
...
if __name__ == "__main__":
    ...
    for query_id, query in query_map.items():
        query2criteria[query] = rank_adapter._generate_adaptive_unit_and_criteria(query=query, instruction=instruction)  # noqa

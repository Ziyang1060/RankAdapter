...
if __name__ == '__main__':
    ...
    run_parser.add_argument('--hits', type=int, default=100, help='Number of hits to rerank.')
    ...
    # Ensure 'run' sub-command is always passed
    if 'run' not in arg_dict or arg_dict['run'] is None:
        raise ValueError("The 'run' sub-command must always be specified.")

    # Ensure exactly one of pointwise, listwise, setwise, bi-encoder, cross-encoder, bm25 is passed
    ...
    # Ensure 'adapter' is only passed when one of pointwise, listwise, or setwise is passed
